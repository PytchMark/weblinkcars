...
exports.handler = async (event) => {
  ...
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    return jsonResponse(500, { ok: false, error: "Supabase service credentials are not configured." });
  }
  ...
  if (userError || !userData?.user) {
    return jsonResponse(403, { ok: false, error: "Admin user not found in Supabase Auth." });
  }

  const { error: profileError } = await supabase
    .from("profiles")
    .upsert({ id: userData.user.id, role: "admin" }, { onConflict: "id" });

  if (profileError) {
    return jsonResponse(500, { ok: false, error: "Failed to ensure admin profile." });
  }
